...
4. Require Codex to sign role-pass with `Executed by: codex` and record `Executor: codex-cli` (or `provider-api`) with the matching `Transport` and `Delegation boundary`.
