...
| Intent resolution          | `providers inspect`, `collaboration plan`, `validate-execution-intent-evidence` | Required when advanced harness behavior or routed agents are used.     | Stop or use only an explicit semantic fallback when a required intent is unavailable.     |
| Role acceptance            | `collaboration classify`, `collaboration verify`, `collaboration validate`      | Required for digest-bound handover acceptance and councils.            | Return rework on stale evidence, failed criteria, or unresolved blocking objections.      |
