...
export function resolveBranchStrategy(config = {}) {
  ...
  return {
    ...
    releaseCandidate: branching.releaseCandidate ?? DEFAULT_BRANCH_STRATEGY.releaseCandidate,
...
export function validateBranchStrategyConfig(config = {}) {
  ...
  if (isPlainObject(branching)) {
    for (const key of ['trunk', 'releaseCandidate', 'integration', 'defaultPrTarget']) {
  ...
  const tiers = [strategy.trunk, strategy.releaseCandidate, strategy.integration]
