...
describe('role routing', () => {
  it('accepts a current role input and emits the same canonical slug', () => {
    const route = resolveRoleRoute({
      role: 'product-manager',
      currentAgent: 'claude',
      config: {},
    })
    expect(route).toMatchObject({ ok: true, role: 'product-manager' })
  })
  ...
  it('rejects removed shell-string availability commands', () => {
    const config = structuredClone(baseConfig)
    config.routing.agents.codex.availabilityCommand = 'exit 99'
    const validation = validateRoutingConfig(config, { checkDocs: false })
    expect(validation.ok).toBe(false)
    expect(validation.errors.join('\n')).toContain('availabilityCommand is unsupported')
  })

  it('uses a non-shell availability probe and falls back when it is unavailable', () => {
    const config = structuredClone(baseConfig)
    config.routing.agents.codex.availabilityProbe = {
      executable: 'agentflow-command-that-does-not-exist',
      args: ['--version'],
    }
    const route = resolveRoleRoute({ role: 'developer', currentAgent: 'claude', config })
    expect(route.selectedAgent).toBe('claude')
    expect(route.fallbacksTried[0]).toMatchObject({
      agent: 'codex',
      available: false,
