...
// W8f D4 — real bilateral role-collaboration wiring (the same shape the "advances a current
// observed candidate" test above builds by hand), factored out so both the escalation test and the
// distinct-errors test below can reach `advance`'s `verifyCriteria` call instead of being stopped
// earlier by "Current bilateral role acceptance is required".
...
describe('governed run service', () => {
  ...
  it('freezes policy before evidence and escalates caller policy weakening to a human gate instead of crashing', async () => {
    // W8f D4 — DEFECT FIXED. This test used to end with `advance` rejecting on 'bilateral': the
    // fixture never satisfied bilateral role-collaboration, so `advance` threw at the
    // "Current bilateral role acceptance is required" check BEFORE it ever called
    // `verifyCriteria` — the test's own name ("...escalates...") was never actually exercised by
    // `advance`. Satisfying human acceptance (already true by default in fixture()) and bilateral
    // acceptance (wired below, the same way the "advances a current observed candidate" test above
    // does) lets `advance` reach `verifyCriteria`, hit the SAME escalated drift, and refuse because
    // of the escalation — which is what this test's name has always claimed.
    ...
    // W8f D4/D5 — `advance` now reaches `verifyCriteria` (bilateral and human acceptance are both
    // satisfied above) and refuses BECAUSE of the escalation, naming the gate class — never the
    // generic "unresolved verification" message a failed piece of evidence would produce (see the
    // distinct-errors test below).
    await expect(service.advance({ contract: plan, authority })).rejects.toThrow('agent-escalation')
  })

  it('W8f D5/D6 — an unresolved escalation and a failed piece of evidence produce different advance errors', async () => {
    // An orchestrator reading `advance`'s error needs to tell "evidence failed" (go get better
    // evidence) from "a human decision is owed" (stop and ask a human) apart. Before the fix both
    // cases threw the exact same message: 'Acceptance blocked by unresolved verification'.
    const { frozen, resolveContract, resolveCollaboration } = bilateralFixtureOverrides()
    const { service } = await fixture({ resolveContract, resolveCollaboration })
    await service.freezeContract({ expectedRevision: (await service.read()).revision, authority })
    await record(service, 'candidate', { digest: candidate })

    // Case 1: the submitted criteria DRIFT from what was frozen -> verifyCriteria escalates to a
    // human gate.
    ...
    const escalationError = await service.advance({ contract: escalationPlan, authority }).then(
      () => null,
      (error) => error.message,
    )
    expect(escalationError).toMatch(/agent-escalation/)

    // Case 2: the submitted criteria match exactly what was frozen (no drift, nothing to escalate)
    // but point at an observation that was never recorded -> the evidence itself is missing/failed.
    const evidencePlan = {
      candidateDigest: candidate,
      runRevision: (await service.read()).revision,
      criteria: [{ ...frozen.criteria[0], observationDigest: 'd'.repeat(64) }],
    }
    const evidenceError = await service.advance({ contract: evidencePlan, authority }).then(
      () => null,
      (error) => error.message,
    )
    expect(evidenceError).toBe('Acceptance blocked by unresolved verification')
    expect(evidenceError).not.toMatch(/agent-escalation/)
    expect(evidenceError).not.toBe(escalationError)
  })
