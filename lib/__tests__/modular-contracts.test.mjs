...
describe('modular AgentFlow contracts', () => {
  ...
  // W8g D1 — validateCollaborationIntent now runs inside createCollaborationIntent itself (its
  // natural enforcement point: the one place a CollaborationIntent record is composed). Every
  // input createCollaborationIntent accepts today is already validated or hardcoded correctly, so
  // there is no adversarial public-API input that reaches this guard; this proves the guard itself
  // actually fires — a regression in the composition logic below (mocked here as a deliberately
  // contract-invalid execution intent) is refused at creation time instead of silently propagating
  // into bindProvider/planBoundExecution downstream.
  it('refuses to create a collaboration intent whose composed record violates the contract', async () => {
    vi.resetModules()
    vi.doMock('../core/execution-intent.mjs', async (importOriginal) => {
      const actual = await importOriginal()
      return { ...actual, createExecutionIntent: () => ({ version: 999 }) }
    })
    const { createCollaborationIntent: createBrokenIntent } =
      await import('../core/collaboration-intent.mjs')
    expect(() => createBrokenIntent({ requestedMode: 'single-agent' })).toThrow(
      /invalid collaboration intent/i,
    )
    vi.doUnmock('../core/execution-intent.mjs')
    vi.resetModules()
  })
  ...
  // W8g D1 — validateProviderBinding now runs inside bindProvider itself (its natural enforcement
  // point: the one place a ProviderBinding record is composed and handed back to a caller). This
  // is a real gap it catches: a provider whose facets do not require 'execution' is never required
  // to declare a `platform` (validateProviderDescriptor only checks that when facets include
  // 'execution'), so it can be accepted as bound/available while
  // `inspected.platform ?? provider.platform ?? null` resolves to null. Before this wiring, that
  // malformed binding was returned to the caller unchecked; now bindProvider refuses it.
  it('refuses to bind a provider whose binding would identify no platform', async () => {
    const provider = {
      version: 1,
      id: 'no-platform-provider',
      ...
      transports: [],
      osSupport: [],
      trust: { source: 'shipped' },
      compatibility: { agentflow: '^1' },
      inspect: async () => ({ availability: 'available' }),
    }
    const intent = { version: 1, execution: { requiredFacets: ['evidence'], requirements: [] } }
    await expect(bindProvider({ intent, providers: [provider] })).rejects.toThrow(
      /invalid provider binding.*platform/i,
    )
  })
