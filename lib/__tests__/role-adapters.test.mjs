...
describe('role adapter projections', () => {
  it('dry-runs without writes and applies all role packages for every harness', () => {
    ...
    // 40 role projections + 20 command projections (claude-code + pi)
    expect(preview.entries.filter((entry) => entry.role)).toHaveLength(60)
    ...
    const claudeCommand = join(targetDir, '.claude', 'commands', 'role-developer.md')
    expect(existsSync(claudeCommand)).toBe(true)
    expect(readFileSync(claudeCommand, 'utf8')).toContain('Role: Developer')
