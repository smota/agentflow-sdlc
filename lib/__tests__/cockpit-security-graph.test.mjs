...
describe('cockpit relationship graph', () => {
  it('derives graph nodes and edges from durable evidence', () => {
    ...
    // W8c D1/D3 — DEFECT FIXED. Unit nodes used to be keyed by the ad hoc issue-colon-number
    // spelling; they are now keyed by the one canonical identity function, namespaced by repo.
    expect(graph.nodes.map((node) => node.id)).toEqual(
      expect.arrayContaining([
        unitIdentity({ repo, id: 119 }),
        unitIdentity({ repo, id: 134 }),
        'comment:1',
        'pr:126',
      ]),
    )
