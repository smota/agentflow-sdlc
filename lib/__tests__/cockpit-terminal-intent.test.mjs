...
describe('close-unit: a terminal intent anchored in the run store', () => {
  ...
  it("6. a human close-unit anchored in the run store completes the cockpit's human gate on the next read; a comment edited after anchoring is detected as divergent and does not complete it", async () => {
    ...
    // W8d D4 — CHANGED (was `toEqual([{ gateClass, subjectDigest }])`, a bare shape with no gate
    // seal and no attestation — exactly the defect this release fixes). resolveAnchoredHumanGate now
    // rebuilds a real sealed adequacy-of-intent gate and returns it paired with the anchored
    // attestation, only after independently re-verifying the pair with satisfyGate itself.
    expect(satisfiedGates).toHaveLength(1)
    expect(satisfiedGates[0].gate).toMatchObject({
      type: 'gate',
      version: 1,
    ...
    expect(satisfiedGates[0].attestation).toEqual(attestationFor(subjectDigest))
    expect(satisfyGate(satisfiedGates[0].gate, satisfiedGates[0].attestation).ok).toBe(true)
    ...
    const gateAfterEdit = deriveHumanGate({
      selectedPath,
      subjectDigest,
      satisfiedGates: afterEdit,
    })
