...
describe('S5 incremental onboarding transactional support', () => {
  ...
  it('Requirement 4 & 6: seedValues shallow merges chosen top-level config preserving unchosen fields', () => {
    const initialConfig = {
      ...
      branching: { trunk: 'old', integration: 'development', strategy: 'workstream' },
    }
    writeFileSync(
    ...
    expect(finalConfig.branching).toEqual({
      trunk: 'main',
  ...
  it('Requirement 7: explicit target root identity binds dev/ino and rejects recreated root while supporting nonexistent targets', () => {
    ...
    try {
      ...
      expect(plan.rootIdentity.birthtimeMs).toBe(statSync(existingRoot).birthtimeMs)

      // Simulate inode reuse with a distinct creation time, without a timing-dependent fixture.
      const { token: originalToken, ...reusedBase } = plan
      reusedBase.rootIdentity = {
        ...plan.rootIdentity,
        birthtimeMs: plan.rootIdentity.birthtimeMs - 1,
      }
      const reused = {
        ...reusedBase,
        token: createHash('sha256').update(JSON.stringify(reusedBase)).digest('hex'),
      }
      expect(() =>
        applyAdoption(packageRoot, existingRoot, reused, { confirm: reused.token }),
      ).toThrow('Adoption target root identity changed; recreated or moved root rejected')

      // Keep the old inode allocated so replacement is deterministic on all filesystems.
      renameSync(existingRoot, `${existingRoot}-original`)
      ...
      rmSync(`${existingRoot}-original`, { recursive: true, force: true })
    }
  })
