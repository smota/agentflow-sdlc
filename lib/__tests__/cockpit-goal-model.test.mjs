...
describe('AgentFlow goal model', () => {
  ...
  it('requires a human gate from a derivable crossing even outside the high-assurance profile', () => {
    // An exploratory path may only propose, never open a PR unattended. Opening one is an
    // authority-escalation crossing that fires regardless of profile — the exact contradiction
    // (bounded/standard/exploratory silently reporting "not-required") this release fixes.
    const selectedPath = deriveSelectedPath({
      issue: issue({ body: '## Acceptance criteria\n- [ ] done\n\nExploratory spike' }),
    })
    expect(selectedPath.profile).toBe('exploratory')
    expect(deriveHumanGate({ selectedPath, comments: [], pullRequests: [] })).toMatchObject({
      required: false,
      status: 'not-required',
    })
    expect(
      deriveHumanGate({ selectedPath, comments: [], pullRequests: [{ number: 1 }] }),
    ).toMatchObject({ required: true, status: 'not-requested' })
  })
})
