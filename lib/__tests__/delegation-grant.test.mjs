...
describe('typed host delegation application boundary', () => {
  ...
  it('rejects prototype parentId and never mints an unconstrained root grant', async () => {
    const f = await serviceFixture()
    for (const protoKey of ['constructor', 'toString', '__proto__']) {
      await expect(
        f.service.issueGrant(request(f.p), await f.opts({ parentId: protoKey })),
      ).rejects.toThrow(/Unknown parent grant ID/)
    }
  })
  it('allows terminal reconciliation of admitted operation even when safety reserve is exhausted by checkpoints', async () => {
    let outcome
    const f = await serviceFixture({ reconcileOperation: async () => outcome })
    const g = await f.service.issueGrant(request(f.p), await f.opts())
    const op = operation(f.p)
    ...
    // Exhaust safety reserve with safety checkpoints (reserve is 2 in default fixture policy)
    await f.service.safetyCheckpoint(
      { id: 'safe-chk-1', kind: 'checkpoint', reason: 'checkpoint 1' },
      await f.opts(),
    )
    await f.service.safetyCheckpoint(
      { id: 'safe-chk-2', kind: 'checkpoint', reason: 'checkpoint 2' },
      await f.opts(),
    )
    // Verify a 3rd checkpoint is rejected by safety reserve
    await expect(
      f.service.safetyCheckpoint(
        { id: 'safe-chk-3', kind: 'checkpoint', reason: 'checkpoint 3' },
        await f.opts(),
      ),
    ).rejects.toThrow(/Safety reserve exhausted/)

    // Now reconcile the admitted operation with confirmed outcome — MUST NOT be rejected
    const admitted = (await f.service.read()).state.admissions[op.id]
    outcome = {
      verified: true,
      state: 'confirmed',
    ...
    const reconciled = await f.service.reconcileDelegatedOperation({
      operationId: op.id,
      authority: f.authority,
    })
    expect(reconciled.state).toBe('confirmed')
    expect((await f.service.read()).state.operations[op.id].state).toBe('confirmed')
  })
})
