...
describe('Harness Execution Contract (S6)', () => {
  ...
  describe('preflightHarnessCapability', () => {
    it('rejects missing provider', async () => {
      await expect(preflightHarnessCapability({ provider: null })).rejects.toThrow(HarnessContractError)
  ...
  describe('validateDispatchContext', () => {
    ...
    it('rejects contexts exceeding 32 KiB', () => {
      ...
      expect(() => validateDispatchContext({ data: oversized })).toThrowError(/exceeds bounded limit/)
    })
  })
  ...
  describe('parseAndVerifyHarnessOutput', () => {
    ...
    it('rejects explicit truncation markers', () => {
      ...
      expect(() => parseAndVerifyHarnessOutput('Some log ... [truncated]')).toThrowError(/truncated/)
      expect(() =>
    ...
    it('rejects unsafe artifact paths', () => {
      ...
      expect(() => parseAndVerifyHarnessOutput(unsafe)).toThrowError(/Invalid or unsafe artifact path/)
    })
    ...
    it('verifies valid artifacts and computes accurate digests', () => {
      ...
      const verified = parseAndVerifyHarnessOutput(payload, { expectedArtifacts: ['lib/hello.mjs'] })
