...
describe('role-attribution', () => {
  ...
  it('accepts registered harness platforms in a matrix while keeping executor distinct', () => {
    const rows = rowsFromMarkdown(
      matrixMarkdown([
        '| 4 | developer | cowork | cowork | pi-subagent-model | forked-context | not-applicable | pass |',
        '| 6 | review | antigravity | antigravity | agy-cli | current-session | independent | pass |',
      ]),
    )
    const result = validateRoleAttributionMatrix({ rows, multiAgentClaim: true })
    expect(result.ok).toBe(true)
  ...
  it('accepts a project-registered future runtime without validator changes', () => {
    const rows = rowsFromMarkdown(
      matrixMarkdown([
        '| 4 | developer | future-runtime | future-runtime | provider-api | provider-api-call | not-applicable | pass |',
        reviewRow('agy'),
      ]),
    )
    const result = validateRoleAttributionMatrix({
  ...
  it('rejects an unregistered platform slug in a matrix row', () => {
    const rows = rowsFromMarkdown(
      matrixMarkdown([
    ...
    const result = validateRoleAttributionMatrix({ rows, multiAgentClaim: true })
    expect(result.ok).toBe(false)
    expect(result.errors.join('\n')).toContain('unregistered actual platform')
  })
