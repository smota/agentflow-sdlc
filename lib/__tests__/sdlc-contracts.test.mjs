...
// Product constants (vocabulary, canonical roles, transitions, principles) no longer live in the
// adopter file itself (see W6a) — they are merged in by loadSdlcConfig at load time. This suite
// exercises the *effective* config every real consumer sees, so it reads through the loader
// rather than parsing defaults/sdlc.config.json directly.
...
describe('canonical SDLC vocabulary', () => {
  ...
  it('includes every configured profile and accepts current role slugs only', () => {
    expect(sdlcVocabulary(config).profiles).toContain('exploratory')
    expect(canonicalRole('product-manager', config)).toBe('product-manager')
    expect(canonicalRole('implementation-planner', config)).toBe('implementation-planner')
    expect(canonicalRole('technical-writer', config)).toBe('technical-writer')
    expect(canonicalRole('developer-plan', config)).toBeNull()
  })
  ...
  it('separates workflow capabilities, tool permissions, and controls', () => {
    ...
    expect(validateRequirementNamespaces({ requiredExecutionIntents: ['shell'] }).ok).toBe(false)
  })
})
        ...
        cases: [
          {
            ...
            assertions: [
              { type: 'contains', value: 'product-manager' },
