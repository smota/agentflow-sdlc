  ...
  it('run-decision invariance holds regardless of telemetry configuration or failure', async () => {
    ...
    const executeWorkload = async (telemetryOpts, failObserver = false) => {
      ...
      const service = createRunService({
        ...
        observeWriter: async () => ({ verified: true, identity: 'test' }),
        observeWorkspace: async () => ({ verified: true, candidateDigest: '123' }),
    ...
    expect(enabledState).toBe(baseState)
    expect(offlineState).toBe(baseState)
    expect(failureState).toBe(baseState)
    expect(baseState).toBe('active') // Should not be 'unknown' or error
  })
})
