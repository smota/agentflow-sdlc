...
describe('current invocation verification', () => {
  ...
  // W8e / D1 — a format whose single assertion passes exactly when the process exits 0, for the
  // overwhelming majority of test scripts that never print JUnit at all.
  describe('the exit-code format (W8e / D1)', () => {
    it('passes its one assertion when the process exits 0', () => {
      const options = fixture()
      options.definition.format = 'exit-code'
      options.definition.args = ['-e', 'process.exit(0)']
      const { observation } = collectProcessObservation(options)
      expect(observation.outcome).toBe('pass')
      expect(observation.assertions).toEqual([{ id: 'search', outcome: 'pass' }])
    })

    it('fails its one assertion when the process exits non-zero, without ever looking for JUnit', () => {
      const options = fixture()
      options.definition.format = 'exit-code'
      options.definition.args = ['-e', 'process.exit(1)']
      const { observation } = collectProcessObservation(options)
      expect(observation.outcome).toBe('fail')
      expect(observation.assertions).toEqual([])
      expect(observation.errors).not.toContain('Unsupported JUnit document')
    })
  })

  // W8e / D2 — the generic "Check process failed, timed out or was interrupted" message conflated
  // four different causes and the observation recorded none of them (no exit code, no signal, no
  // spawn error code, no stderr). These tests write distinct causes through the SAME injectable
  // `runner` seam the rest of this file already uses (matching how spawnSync actually reports each
  // case, confirmed by direct reproduction on a fresh repository — see the session report) and check
  // that the record and the message both name the real cause, not a guess.
  describe('the real cause of a failed check is recorded and named (W8e / D2)', () => {
    it('names a process that could not start, with its spawn error code', () => {
      const options = fixture()
      const result = collectProcessObservation({
        ...options,
        runner: () => ({
          ...
          stdout: '',
          stderr: '',
          error: Object.assign(new Error('spawnSync fixture ENOENT'), { code: 'ENOENT' }),
        }),
      })
      expect(result.observation.outcome).toBe('fail')
      expect(result.observation.errors).toContain('Check process could not start (ENOENT)')
      expect(result.observation.spawnErrorCode).toBe('ENOENT')
    ...
    it('names a process that exited non-zero, with the exact code', () => {
      const options = fixture()
      const result = collectProcessObservation({
        ...options,
        runner: () => ({ status: 1, signal: null, stdout: '', stderr: 'boom', error: undefined }),
      })
      expect(result.observation.outcome).toBe('fail')
      expect(result.observation.errors).toContain('Check process exited with code 1')
      expect(result.observation.exitCode).toBe(1)
      expect(result.observation.spawnErrorCode).toBeNull()
      expect(result.observation.timedOut).toBe(false)
    })

    it('names a process that timed out, distinctly from one that could not start or merely exited non-zero', () => {
      const options = fixture()
      const result = collectProcessObservation({
        ...options,
        runner: () => ({
          ...
          stdout: '',
          stderr: '',
          error: Object.assign(new Error('spawnSync fixture ETIMEDOUT'), { code: 'ETIMEDOUT' }),
        }),
      })
      expect(result.observation.outcome).toBe('fail')
      expect(result.observation.errors).toContain(
        `Check process timed out after ${options.definition.timeoutMs}ms`,
      )
      expect(result.observation.timedOut).toBe(true)
      expect(result.observation.spawnErrorCode).toBe('ETIMEDOUT')
    })

    it('names a process killed by a signal, distinctly from a timeout', () => {
      const options = fixture()
      const result = collectProcessObservation({
        ...options,
        runner: () => ({ status: null, signal: 'SIGKILL', stdout: '', stderr: '', error: undefined }),
      })
      expect(result.observation.outcome).toBe('fail')
      expect(result.observation.errors).toContain('Check process was killed by signal SIGKILL')
      expect(result.observation.signal).toBe('SIGKILL')
      expect(result.observation.timedOut).toBe(false)
      expect(result.observation.spawnErrorCode).toBeNull()
    })

    it('records the exit code and a bounded stderr excerpt on the observation (W8e / D2, test 3)', () => {
      const options = fixture()
      const result = collectProcessObservation({
        ...options,
        runner: () => ({
      ...
      expect(result.observation.exitCode).toBe(1)
      expect(result.observation.stderrExcerpt).toContain('assertion failed at line 12')
    })
  })
})
