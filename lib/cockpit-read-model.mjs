...
import { buildAgentFlowGoalModel, buildCommandCenterModel } from './cockpit-goal-model.mjs'
...
export function buildCockpitIssueView({
  ...
  reviewFindings = [],
  repo,
  // W8c D5 — satisfied gates the caller resolved from the durable run store (see
  // lib/cockpit-intent-anchor.mjs's resolveAnchoredHumanGate, called from
  // scripts/cockpit-server.mjs). This read-model function stays pure/synchronous: it never does the
  // I/O itself, it only threads the result through to deriveHumanGate.
  satisfiedGates = [],
} = {}) {
  ...
  const graph = buildCockpitGraph({ issue, comments, pullRequests, repo })
  ...
  return {
    ...
    graph,
    goal,
    nextSafeAction: goal.nextBestActions[0]?.id || phaseState.nextSafeAction,
  }
}

export function buildGoalBoard({
  issues = [],
  commentsByIssue = {},
  pullRequestsByIssue = {},
  repo,
} = {}) {
  return buildCommandCenterModel({ issues, commentsByIssue, pullRequestsByIssue, repo })
}
