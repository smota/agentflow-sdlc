...
// comment after the fact — or no matching comment exists any more — the decision is not honored. On
// a match, this synthesizes the ONE satisfiedGates entry `deriveHumanGate`
// (lib/cockpit-goal-model.mjs, W8b D2) already knows how to consume; no new completion mechanism,
// the existing one finally gets fed real input. Called from scripts/cockpit-server.mjs's issue page.
export async function resolveAnchoredHumanGate({
  ...
  return [{ gateClass: GATE_CLASSES.adequacyOfIntent, subjectDigest: anchored.subjectDigest }]
}
