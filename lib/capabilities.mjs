...
// Compatibility alias retained for consumers of the pre-namespace API.
export const CAPABILITIES = WORKFLOW_CAPABILITIES
...
export function isKnownCapability(capability) {
  return WORKFLOW_CAPABILITIES.includes(capability)
}

export function classifyLegacyCapability(value) {
  if (WORKFLOW_CAPABILITIES.includes(value)) return 'workflow-capability'
  if (TOOL_PERMISSIONS.includes(value)) return 'tool-permission'
  if (CONTROL_REQUIREMENTS.includes(value)) return 'control-requirement'
  return null
}
...
export function validateRequirementNamespaces(requirements = {}) {
  ...
  const warnings = []
  const fields = [
    ['requiredWorkflowCapabilities', WORKFLOW_CAPABILITIES],
  ...
  for (const value of requirements.requiredCapabilities ?? []) {
    const classification = classifyLegacyCapability(value)
    if (!classification) errors.push(`requiredCapabilities contains unsupported value: ${value}`)
    else
      warnings.push(
        `requiredCapabilities is deprecated; move ${value} to the ${classification} namespace`,
      )
  }
