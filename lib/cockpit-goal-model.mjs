...
import { deriveCrossings } from './core/gate.mjs'
import {
...
export function buildAgentFlowGoalModel({
  ...
  // Identity is the content digest, computed with the same helper and field set as the run track's
  // goalRevision (lib/core/record-digest.mjs, recordDigest({repo, number, title, body, updatedAt}) in
  // scripts/run-delivery.mjs). `id` below is a human-facing display label only; anything keying,
  // comparing or deduplicating intents must use `revision`.
  const revision = recordDigest({
    repo,
    number: issue.number,
    title: issue.title,
    body: issue.body,
  ...
  return {
    ...
    // native hierarchy — see extractIssueRelationships). Its `revision` is resolved once all intents
    // in a batch are known (resolvePartOf, called from buildCommandCenterModel); a standalone build
    // only knows the parent's issue number.
    partOf: relationships.partOfIssue
      ? { number: relationships.partOfIssue, revision: null }
      : null,
    status,
    selectedPath,
...
export function resolvePartOf(goals = []) {
  const byNumber = new Map(goals.map((goal) => [goal.number, goal]))
  return goals.map((goal) => {
    if (!goal.partOf) return goal
    const parent = byNumber.get(goal.partOf.number)
    return { ...goal, partOf: { number: goal.partOf.number, revision: parent?.revision ?? null } }
  })
}
...
// This answers "does a human have to act here?" from actual gates (lib/core/gate.mjs), never from
// regex over comment prose. `required` no longer collapses to "profile is high-assurance" alone: a
// derivable crossing (deriveCrossings) fires independently of profile, so e.g. an exploratory issue
// that has opened a PR — exceeding what an exploratory path may do unattended — also requires a
// human, exactly like the bounded-run-taking-an-external-action contradiction this replaces.
// `complete` is decided only by a structural, unforgeable signal (an actual GitHub review decision)
// — never by an agent's own comment text claiming approval. That prose match was the invariant-9
// hole: a comment saying "approved by human" used to satisfy the gate by itself.
export function deriveHumanGate({ selectedPath, comments = [], pullRequests = [] } = {}) {
  const requestedBoundary = DEFAULT_PROFILE_MAXIMUMS[selectedPath?.profile] ?? 'open-pr'
  const effectiveBoundary = pullRequests.length ? 'open-pr' : 'observe'
  const crossings = deriveCrossings({ effectiveBoundary, requestedBoundary })
  const required = selectedPath?.profile === 'high-assurance' || crossings.length > 0
  const complete = pullRequests.some((pr) => pr.reviewDecision === 'APPROVED')
  ...
  return {
    required,
    reason: required
      ? (crossings[0]?.reason ?? 'High-assurance path requires a human approval gate.')
