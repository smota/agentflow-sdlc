...
import { deriveCrossings, GATE_CLASSES, satisfyGate } from './core/gate.mjs'
...
const FALLBACK_POSTURE_CONFIG = Object.freeze({
  paths: Object.freeze({
    light: Object.freeze({ allowsSelfReview: true, requiresHumanApproval: false }),
...
export function deriveSelectedPath({ issue = {}, comments = [], pullRequests = [] } = {}) {
  ...
  const explicitProfile = fieldMatch(
    body,
    /(profile|workflow classification|path|assurance path)\s*:?\s*(light|bounded|standard|high-assurance|exploratory)/i,
  )
  const profile = explicitProfile || profileFromSignals({ body, labels: labelNames, pullRequests })
  const explicitRisk = fieldMatch(body, /(risk level|risk)\s*:?\s*(low|medium|high|critical)/i)
  const risk = explicitRisk || riskFromSignals({ body, labels: labelNames, profile })
...
export function deriveHumanGate({
  ...
  const required = requiredHumanGateClasses({ posture, changeClass, config, crossings }).length > 0
  // W8d D4 — DEFECT FIXED. This used to complete on the SHAPE of a `satisfiedGates` entry alone: any
  // plain object with a matching `gateClass` and `subjectDigest` completed the gate, re-verifying
  // nothing (no `satisfyGate`, no attestation, no seal — a forged or malformed record was
  // indistinguishable from a real one). `satisfiedGates` is now a list of `{ gate, attestation }`
  // pairs; a pair only completes the gate when `gate` is itself a sealed adequacy-of-intent gate over
  // THIS subject AND `satisfyGate` (lib/core/gate.mjs — read-only, called here, never reimplemented)
  // accepts the pair. `satisfyGate` re-verifies the gate's own seal, the attestation's shape and
  // digest match, that the decision is `agree`, and that the reviewer is an actual human — a
  // well-formed record is not consent; only a satisfied gate is.
  const complete = satisfiedGates.some((entry) => {
    const gate = entry?.gate
    if (gate?.gateClass !== GATE_CLASSES.adequacyOfIntent) return false
    if (subjectDigest == null || gate?.subjectDigest !== subjectDigest) return false
    return satisfyGate(gate, entry?.attestation).ok
  })
...
export function deriveNextBestActions({
  ...
  if (relationships?.blockedBy?.length) {
    const openBlockers = relationships.blockedBy.filter((blockerNum) => {
      const blocker = allIssues.find((i) => i.number === blockerNum)
      return blocker ? blocker.state !== 'closed' : true
    })
    if (openBlockers.length) {
      actions.push(
        action(
          ...
          `Goal is blocked by open issue(s): ${openBlockers.map((num) => `#${num}`).join(', ')}.`,
          98,
        ),
      )
    }
...
function skipReason({ phase, profile, risk }) {
  if (profile === 'bounded' || profile === 'light')
    return `${phase.label} skipped by ${profile} low-risk path; no readiness penalty.`
