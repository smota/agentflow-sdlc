...
export const ROLE_REQUIREMENTS = {
  ...
  'high-assurance': {
    required: COCKPIT_PHASES.map((phase) => phase.slug),
    optional: [],
  },
  exploratory: {
...
export function buildReleaseDashboard(goals = []) {
  ...
  const releaseCandidate =
    goals.find((goal) => goal.versionLens.candidateVersion)?.versionLens.candidateVersion ||
    `next ${goals[0]?.versionLens?.targetBranch || 'release'}`
  const targetBranch =
...
export function deriveHumanGate({ selectedPath, comments = [], pullRequests = [] } = {}) {
  ...
  return {
    required,
    reason: required
      ? 'High-assurance path requires explicit human review.'
      : 'Selected path does not require a human gate.',
    status,
    nextAction:
      required && !complete
        ? requested
          ? 'Track human review to completion.'
...
export function deriveVersionLens({
  ...
  const releaseImpact = releaseImpactFromSignals({ body, labels })
  const awaitingRelease =
    labels.includes('awaiting-release') || Boolean(mergedPr && targetBranch === 'development')
  const candidateVersion = body.match(/v\d+\.\d+\.\d+/i)?.[0] || null
...
export function buildReadinessHealth({
  ...
  const dimensions = [
    ...
    scoredDimension(
      ...
      selectedPath.applicableRoles.includes('review'),
      'Review readiness',
  ...
  if (selectedPath.profile === 'high-assurance' && !hasHumanGateSignal(comments))
    actions.push(
      action(
        'request-human-gate',
        'Request human review',
        'Human review is required for this selected path and is not complete yet.',
        95,
      ),
    )
...
function releaseImpactFromSignals({ body, labels }) {
