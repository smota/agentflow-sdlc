...
export function buildAgentFlowGoalModel({
  ...
  // scripts/run-delivery.mjs). Computed here (rather than further down) because deriveHumanGate needs
  // it: an adequacy-of-intent gate always ranges over the goal's OWN revision (lib/core/gate.mjs), so
  // that is the subject a satisfied gate must match to complete the human gate below (W8b D2).
  ...
  // adequacy-of-intent gate's subject. `id` below is a human-facing display label only; anything
  // keying, comparing or deduplicating intents must use `revision`.
  return {
