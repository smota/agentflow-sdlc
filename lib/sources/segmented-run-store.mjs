    ...
    if (manifestEntry) {
      ...
      for (const seg of manifest.segments) {
        ...
        if (computedDigest !== seg.digest)
          throw new Error(
            `Segment digest mismatch: expected ${seg.digest}, computed ${computedDigest}`,
          )
  ...
  return {
    ...
    async append(event, expectedRevision) {
      ...
      if (tailEvents.length >= segmentSize || tailBytes >= segmentBytes) {
        ...
        const startState =
          existingSegmentEventCount > 0
            ? reduceRun(allEvents.slice(0, existingSegmentEventCount))
            : null
        const endState = reduceRun(
          allEvents.slice(0, existingSegmentEventCount + tailEvents.length),
        )
