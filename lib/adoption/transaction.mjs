...
function readLockForPlan(root, { migrateLegacy = false } = {}) {
  ...
  try {
    ...
    throw new Error(`Invalid AgentFlow lockfile: ${error.message}`)
  }

  if (parsed?.version === 2) {
    return readLockfile(root, { strict: true })
  }
  ...
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('Invalid AgentFlow lockfile: legacy lockfile must be an object')
  }
  ...
  if (!parsed.files || typeof parsed.files !== 'object' || Array.isArray(parsed.files)) {
    throw new Error('Invalid AgentFlow lockfile: legacy files must be an object')
  }
  if (!Array.isArray(parsed.merged)) {
    throw new Error('Invalid AgentFlow lockfile: legacy merged must be an array')
  }
...
export function planAdoption(
  packageRoot,
  targetDir,
  { profile, storage = 'external', resolutions = {}, seedValues = {}, migrateLegacy = false } = {},
) {
  ...
  const lock = readLockForPlan(root, { migrateLegacy })
...
function applyAdoptionUnlocked(
  ...
  try {
    ...
    const priorLock = readLockForPlan(root, { migrateLegacy: Boolean(plan.options?.migrateLegacy) })
