...
export function buildCockpitGraph({
  ...
  const nodes = []
  const edges = []
  // D1/D3 (W8c) — unit nodes are keyed by the ONE canonical identity, never the ad hoc
  // issue-colon-number spelling this replaced: two repositories with the same issue number must
  // not collide in the same graph.
  const issueId = unitIdentity({ repo, id: issue.number })
  ...
  for (const ref of relationships.issueRefs.filter((number) => number !== issue.number)) {
    const id = unitIdentity({ repo, id: ref })
  ...
  for (const number of [...new Set([...relationships.followUps, ...followUps])]) {
    const id = unitIdentity({ repo, id: number })
