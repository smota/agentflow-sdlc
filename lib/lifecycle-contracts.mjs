...
export function validateExternalSignal(signal = {}, config = {}) {
  ...
  if (['triaged', 'accepted', 'rejected'].includes(signal.state)) {
    if (!['product-manager', 'analyst'].includes(triageRole)) {
      errors.push('triageRole must be product-manager or analyst after triage')
    }
  }
...
export function validateDeliveryHandoff(handoff = {}, config = {}) {
  ...
  if (['ready', 'accepted', 'completed'].includes(handoff.state)) {
    ...
    if (!['pr-readiness', 'technical-writer'].includes(sourceRole)) {
      errors.push('ready delivery handoff must originate from pr-readiness or technical-writer')
    }
...
export function validateActionBoundary(record = {}, config = {}) {
  ...
  if (record.effective === 'external-action') {
    ...
    // Authorising an external action is not performing it. The SDLC still never executes: it
    // requires evidence that the outward handoff (validateDeliveryHandoff, above) has actually
    // been made, in addition to the human approval already required. Both are records of a
    // decision, never the action itself.
    if (!record.deliveryHandoff || typeof record.deliveryHandoff !== 'object') {
      errors.push('external-action requires a delivery handoff')
    } else {
      const handoff = validateDeliveryHandoff(record.deliveryHandoff, config)
      errors.push(...handoff.errors.map((error) => `deliveryHandoff.${error}`))
      warnings.push(...handoff.warnings.map((warning) => `deliveryHandoff.${warning}`))
    }
  }
