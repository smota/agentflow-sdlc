...
export function validateExtensionPack(pack, options = {}) {
  ...
  const requirements = validateRequirementNamespaces({
    requiredCapabilities: asArray(manifest.requiredCapabilities),
    requiredWorkflowCapabilities: asArray(manifest.requiredWorkflowCapabilities),
  ...
  for (const [index, play] of asArray(manifest.plays).entries()) {
    ...
    } else if (play.role && play.role !== playRole) {
      warnings.push(`${prefix}.role is a deprecated alias; emit ${playRole}`)
    }
