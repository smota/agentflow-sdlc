...
import { validateRequirementNamespaces } from './core/execution-intent.mjs'
import { canonicalRole } from './sdlc-vocabulary.mjs'
...
function extensionSdlcConfig(options = {}) {
  if (options.sdlcConfig) return options.sdlcConfig
  const repoRoot = options.repoRoot ?? process.cwd()
  if (
    fs.existsSync(path.join(repoRoot, 'sdlc.config.json')) ||
    fs.existsSync(path.join(repoRoot, 'defaults', 'sdlc.config.json'))
  ) {
    return loadSdlcConfig(repoRoot)
  }
  return loadSdlcConfig(process.cwd())
}
...
export function validateExtensionPack(pack, options = {}) {
  ...
  const requirements = validateRequirementNamespaces({
    requiredExecutionIntents: asArray(manifest.requiredExecutionIntents),
    requiredToolPermissions: asArray(manifest.requiredToolPermissions),
    controlRequirements: asArray(manifest.controlRequirements),
  })
  if (
    manifest.requiredCapabilities !== undefined ||
    manifest.requiredWorkflowCapabilities !== undefined
  ) {
    errors.push(
      `${pack.relativeDir}: untyped or legacy capability requirements are unsupported; use requiredExecutionIntents`,
    )
  }
  errors.push(...requirements.errors.map((error) => `${pack.relativeDir}: ${error}`))
  warnings.push(...requirements.warnings.map((warning) => `${pack.relativeDir}: ${warning}`))

  const config = extensionSdlcConfig(options)
  const playIds = new Set()
  for (const [index, play] of asArray(manifest.plays).entries()) {
    const prefix = `${pack.relativeDir}: plays[${index}]`
    if (!play || typeof play !== 'object' || Array.isArray(play)) {
      errors.push(`${prefix} must be an object`)
      continue
    }
    for (const field of ['id', 'role', 'appliesWhen']) {
      if (typeof play[field] !== 'string' || !play[field].trim()) {
        errors.push(`${prefix}.${field} is required`)
      }
    }
    if (play.id && playIds.has(play.id)) errors.push(`${prefix}.id must be unique: ${play.id}`)
    if (play.id) playIds.add(play.id)
    const playRole = canonicalRole(play.role, config)
    if (play.role && !playRole) {
      errors.push(`${prefix}.role is not a core role: ${play.role}`)
    }
    for (const field of ['requiredInputs', 'expectedOutputs', 'evidenceFields', 'validators']) {
      if (!Array.isArray(play[field]) || play[field].some((value) => typeof value !== 'string')) {
        errors.push(`${prefix}.${field} must be an array of strings`)
      }
    }
    for (const validator of play.validators ?? [])
      checkPath(errors, pack, validator, 'play validator')
    if (play.parameters !== undefined && play.method === undefined) {
      errors.push(`${prefix}.parameters requires method`)
    }
    if (play.method !== undefined) {
      if (typeof play.method !== 'string' || !play.method.trim()) {
        errors.push(`${prefix}.method must be a non-empty string`)
      } else if (
        play.parameters !== undefined &&
        (!play.parameters || typeof play.parameters !== 'object' || Array.isArray(play.parameters))
      ) {
        errors.push(`${prefix}.parameters must be an object when present`)
      } else {
        try {
          const packageRoot = options.repoRoot ?? process.cwd()
          const roleCatalog = loadRoleCatalog(packageRoot)
          const methodCatalog = loadMethodCatalog(packageRoot)
          const roleIdentity = canonicalRoleIdentity(play.role, roleCatalog)
          if (!roleIdentity) throw new Error(`unknown product role ${play.role}`)
          resolveRoleContract({
            role: roleIdentity,
            config: {
              roleMethods: {
                bindings: {
                  [roleIdentity]: [{ method: play.method, parameters: play.parameters ?? {} }],
                },
              },
            },
            packageRoot,
          ...
          errors.push(`${prefix}.method is invalid: ${error.message}`)
        }
      }
    }
    for (const forbidden of ['transitions', 'owner', 'readinessDenominator', 'approvalRules']) {
      if (play[forbidden] !== undefined) {
        errors.push(`${prefix}.${forbidden} cannot override core workflow authority`)
      }
    }
  }
...
export function buildExtensionRegistry(repoRoot = process.cwd(), options = {}) {
  ...
  for (const packPath of configured) {
    ...
    const validation = validateExtensionPack(pack, { ...options, repoRoot })
