...
export function renderIssueView(view) {
  ...
  <section class="panel highlight"><div><h2>Next-best-action</h2>${renderNextActions(goal.nextBestActions.map((action) => ({ ...action, goal })).slice(0, 3))}</div>${renderHealthOrb(goal.evidenceHealth)}</section>
  <section class="panel"><h2>Selected Path</h2>${renderSelectedPath(goal.selectedPath)}</section>
  <section class="panel"><h2>Readiness Health</h2>${renderEvidenceHealthCheck(goal.evidenceHealth)}</section>
  <section class="panel"><h2>Version / Release Lens</h2>${renderVersionLens(goal.versionLens)}</section>
...
@media(max-width:1000px){ .command-workspace{grid-template-columns:1fr} .nav-pane,.action-pane{position:static} }
