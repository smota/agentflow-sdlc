...
export function renderGoalBoard(commandCenter = {}) {
  ...
  return `<section class="metrics">${metric('Active goals', metrics.activeGoals)}${metric('Need attention', metrics.needAttention, 'warn')}${metric('Ready for review', metrics.readyForReview, 'ok')}${metric('Readiness health', `${metrics.evidenceHealthAverage || 0}/100`)}${metric('Awaiting release', metrics.awaitingRelease)}${metric('Human gates', metrics.humanGatesPending, 'warn')}</section>
...
export function renderIssueView(view) {
  ...
  <section class="panel compact-hero"><div><p class="eyebrow">Goal detail</p><h1>${escapeHtml(goal.title)}</h1><p><span class="badge ${stateClass(goal.status)}">${escapeHtml(displayState(goal.status))}</span> <span class="badge">${escapeHtml(goal.goalType)}</span> <a href="${escapeHtml(goal.durableLinks.issue || '#')}">Source</a> <a class="action-link" href="/issues/${goal.number}/replay">↺ Goal Story</a></p></div>${renderHealthOrb(goal.evidenceHealth)}</section>
...
function renderEvidenceHealthCheck(health) {
  return `<div class="health-summary">${renderHealthOrb(health)}<div><p>Score uses ${health.denominator || health.dimensions.length} applicable dimension(s). Skipped or not-applicable checks are excluded.</p><div class="health-dimensions">${health.dimensions
    .map(
      (dimension) =>
    ...
    .map((event) => {
      const badge =
        event.evidenceStatus && event.evidenceStatus !== 'durable'
          ...
          : ''
      return `<li>${badge}<strong>${escapeHtml(event.type)}</strong> ${escapeHtml(event.summary)}${event.evidence?.url ? ` <a href="${escapeHtml(event.evidence.url)}">source</a>` : ''}</li>`
    })
    .join('')}</ol>`
}
...
function renderHealthOrb(health) {
  return `<details class="health-popover"><summary class="health-orb ${health.grade}"><strong>${health.score}</strong><span>${escapeHtml(health.grade)}</span></summary><div class="floating-health"><h3>Readiness details</h3><p>${health.denominator || health.dimensions.length} applicable dimension(s). Skipped checks excluded.</p><ul>${health.dimensions.map((dimension) => `<li><strong>${escapeHtml(dimension.label)}</strong>: ${dimension.score}/100 — ${escapeHtml(displayState(dimension.state))}</li>`).join('')}</ul></div></details>`
}
...
.dimension { border-bottom:1px solid var(--line); padding:.55rem 0; } .comment { border-left: 4px solid var(--teal); padding-left: .75rem; color: var(--muted); } .subtle { background:#fbfdfc; color:var(--muted); } .role-card { position:relative; padding-left:3.25rem; } .role-state { position:absolute; left:1rem; top:1rem; width:1.5rem; height:1.5rem; border-radius:50%; display:grid; place-items:center; font-weight:900; border:1px solid var(--line); } .role-state.ok { color:var(--teal); border-color:var(--teal); } .role-state.warn { color:var(--coral); border-color:var(--coral); } .role-state.bad { color:#b42318; border-color:#b42318; }
...
@media(max-width:820px){ .metrics{grid-template-columns:repeat(2,1fr)} .highlight,.health-summary,.graph-layout,.compact-hero{grid-template-columns:1fr} .summary-strip,.next-action-cards{grid-template-columns:1fr} }
`
