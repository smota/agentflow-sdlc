...
export function renderGoalBoard(commandCenter = {}) {
  ...
  return `<section class="panel hero"><p class="eyebrow">AgentFlow Cockpit</p><h1>Goal Command Center</h1><p>Goals, role flow, readiness, version state, follow-ups, and next-best-actions — synthesized from delivery records.</p></section>
  <section class="metrics">${metric('Active goals', metrics.activeGoals)}${metric('Need attention', metrics.needAttention, 'warn')}${metric('Ready for review', metrics.readyForReview, 'ok')}${metric('Readiness health', `${metrics.evidenceHealthAverage || 0}/100`)}${metric('Awaiting release', metrics.awaitingRelease)}${metric('Human gates', metrics.humanGatesPending, 'warn')}</section>
...
function renderGoalCard(goal) {
  return `<article class="goal-card"><div><span class="badge ${stateClass(goal.status)}">${escapeHtml(displayState(goal.status))}</span><span class="badge">${escapeHtml(goal.selectedPath.profile)}</span><span class="badge">${escapeHtml(goal.versionLens.state)}</span><h3><a href="/issues/${goal.number}">#${goal.number} ${escapeHtml(goal.title)}</a></h3><p>${escapeHtml(goal.highlights?.[0]?.value || '')}</p></div><div class="mini-score">${goal.evidenceHealth.score}</div><p><a href="/issues/${goal.number}/replay">Goal Story</a></p></article>`
}
...
function renderEvidenceHealthCheck(health) {
  return `<div class="health-summary"><div class="health-orb ${health.grade}"><strong>${health.score}</strong><span>${escapeHtml(health.grade)}</span></div><div><p>Score uses ${health.denominator || health.dimensions.length} applicable dimension(s). Skipped or not-applicable checks are excluded.</p><div class="health-dimensions">${health.dimensions
    .map(
      (dimension) =>
