...
export async function preflightHarnessCapability({
  ...
  if (
    executionTarget &&
    Array.isArray(provider.targets) &&
    !provider.targets.includes(executionTarget)
  ) {
    throw new HarnessContractError(
      `Provider does not support executionTarget: ${executionTarget}`,
      {
        code: 'UNSUPPORTED_TARGET',
        details: { supported: provider.targets, requested: executionTarget },
      },
    )
  }
  ...
  if (byteLength > MAX_CONTROL_CONTEXT_BYTES) {
    throw new HarnessContractError(
      `Control context exceeds bounded limit of ${MAX_CONTROL_CONTEXT_BYTES} bytes (actual: ${byteLength} bytes)`,
      {
        ...
        details: { actual: byteLength, max: MAX_CONTROL_CONTEXT_BYTES },
      },
    )
  }
}
...
export function parseAndVerifyHarnessOutput(rawOutput, { expectedArtifacts = [] } = {}) {
  ...
  if (typeof rawOutput === 'object' && rawOutput.truncated === true) {
    throw new HarnessContractError(
      'Harness output was truncated; rejected to prevent corrupted application',
      {
...
export async function dispatchToHarness({
  ...
  const requiresEdit = requiredCapabilities.some((c) =>
    ['file-edit', 'commit', 'workspace-edit'].includes(c),
  )
  if (requiresEdit && permissionBoundary === 'observe') {
    throw new HarnessContractError(
      'Edit capability requested but permission boundary is observe-only',
      {
  ...
  const parsedOutput = parseAndVerifyHarnessOutput(receipt.output?.stdout || receipt.output || {}, {
    expectedArtifacts,
  })
