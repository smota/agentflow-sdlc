...
export function parseMeshloopOutput(stdout) {
  if (typeof stdout !== 'string') {
    throw new MeshloopAdapterError('Meshloop stdout must be a string', { code: 'INVALID_OUTPUT_TYPE' })
  }
  const byteLength = Buffer.byteLength(stdout)
  if (byteLength > MAX_OUTPUT_FRAME_BYTES) {
    throw new MeshloopAdapterError(`Meshloop output exceeds frame limit of ${MAX_OUTPUT_FRAME_BYTES} bytes`, {
...
export function createMeshloopProvider({
  ...
  intentSupport = [
    { id: 'plan-before-edit', implementation: 'native', fidelity: 'full', evidence: 'contract-tested', limits: {} },
    { id: 'workflow-orchestration', implementation: 'native', fidelity: 'full', evidence: 'contract-tested', limits: {} },
    { id: 'bounded-loop', implementation: 'native', fidelity: 'full', evidence: 'contract-tested', limits: {} },
    { id: 'isolated-workspace', implementation: 'native', fidelity: 'full', evidence: 'contract-tested', limits: {} },
  ],
  ...
  const plan = (request = {}) => {
    ...
    if (request.restart === true || request.reset === true || request.flags?.includes('--restart') || request.flags?.includes('--reset')) {
  ...
  const execute = async (executionPlan, { confirm } = {}) => {
    ...
    emitObservation(observer, { kind: 'meshloop_execution_attempt', state: 'started', operationId: token })
  ...
  return {
    ...
    cancel: () => ({ status: 'unsupported', reason: 'Meshloop adapter CLI cancellation operates via OS process signals' }),
