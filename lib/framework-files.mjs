...
//
// Known gap: `sync` does NOT currently perform this seed step (only `init` does), so a project
// that adopts via `sync` (because it already has some conflicting FRAMEWORK_FILES) never gets
// docs/stack-conventions.md seeded automatically — copy it from
// agents/templates/stack-conventions.md by hand in that case. See
// https://github.com/smota/multi-agent-sdlc/issues (filed as a follow-up).
