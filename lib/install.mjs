...
import { buildExtensionRegistry } from './extension-packs.mjs'
...
function extensionStatus(targetDir) {
  const registry = buildExtensionRegistry(targetDir)
  return {
    extensionEnabledValid: registry.enabledValid,
    extensionEnabledMissing: registry.enabledMissing,
    extensionEnabledInvalid: registry.enabledInvalid,
    extensionDiscoveredDisabled: registry.discoveredDisabled,
    extensionDuplicateIds: registry.duplicateIds,
  }
}
...
export function init(packageRoot, targetDir) {
  ...
  return { installed, seeded, seededSkipped, ...extensionStatus(targetDir) }
}
...
export function sync(packageRoot, targetDir) {
  ...
  Object.assign(report, extensionStatus(targetDir))
  writeLockfile(targetDir, lockfile)
  return report
}
