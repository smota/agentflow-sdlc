...
import { WORKFLOW_ROLES } from './role-routing.mjs'
import { runtimePlatformSlugs } from './runtime-platforms.mjs'
...
/**
 * Derive independenceBoundary from evidence rather than accepting it as a claim: given the set of
 * actor identities that authored the reviewed subject and the reviewer's own identity, an
 * "independent" review requires those sets to be disjoint. Any intersection is a "self-review",
 * regardless of what the role pass declares. Returns null when there is no reviewer identity to
 * evaluate against.
 */
export function deriveIndependenceBoundary({ authorIdentities = [], reviewerIdentity } = {}) {
  if (!reviewerIdentity) {
    return null
  }

  const authors = new Set(authorIdentities.filter(Boolean))
  return authors.has(reviewerIdentity) ? 'self-review' : 'independent'
}
...
export function validateRoleAttributionMatrix({
  ...
  let validPlatforms
  try {
    validPlatforms = new Set(runtimePlatformSlugs(platformConfig))
  } catch (error) {
  ...
  for (const row of rows) {
    ...
    } else if (!validPlatforms.has(row.plannedOwner)) {
      errors.push(
        `role attribution matrix row "${label}" has an unregistered planned owner platform: ${row.plannedOwner}; register it in manifests/runtime-platforms.json or platformRegistry.additionalPlatforms`,
      )
    }
    if (!row.actualAgent) {
      errors.push(`role attribution matrix row "${label}" is missing an actual platform`)
    } else if (!validPlatforms.has(row.actualAgent)) {
      errors.push(
        `role attribution matrix row "${label}" has an unregistered actual platform: ${row.actualAgent}; register it in manifests/runtime-platforms.json or platformRegistry.additionalPlatforms`,
      )
    }
  ...
  if (developerRow?.actualAgent && reviewRow?.actualAgent) {
    ...
    if (sameIntelligence) {
      ...
      errors.push(
        'review row is marked "self-review" but its actual platform differs from developer row; independence boundary must be "independent"',
      )
    }
  }
