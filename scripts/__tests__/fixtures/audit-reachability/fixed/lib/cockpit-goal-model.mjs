...
// Fixed: completion is decided by the digest-bound, staleness-checked review attestation
// (lib/core/review-attestation.mjs), never by a prose regex over comment text.
export function deriveHumanGate({
  ...
  const attested = attestation
    ? validateReviewAttestation(attestation, { expectedDigest }).ok &&
      attestation.decision === 'agree'
    : false
  const complete = attested || pullRequests.some((pr) => pr.reviewDecision === 'APPROVED')
